...
def deserialize_usertype(parser: PacketParser):
    ...
    if typename == "ll_access::DramIdentifier":
        ...
        ram_id = bytes(parser.read_bytes(8))
    ...
    if typename == "QVector<QPair<ushort,uchar> >":
        ...
        return vector
    if typename == "QVector<LLMemoryModule>":
        # TODO: properly decode memory module metadata
        vector_size = parser.read_uint(4)
        vector = []
        for _ in range(vector_size):
            vector.append(bytes(parser.read_bytes(253)))
        return vector
...
async def main(args: List[str] = None):
    ...
    arg_parser.add_argument("-w", "--wait", action='store_true', help="Wait for the user to manually start the service")
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        if options.wait:
            print("Please start the service, and press ENTER")
            input()
        else:
